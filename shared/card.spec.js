...
test('A card is ready when played', () => {
  ...
  expect(card.props.exhausted).toBeFalsy();
});

test('A quest card has a unique image front and back', () => {
  let card = library.getCardByName("Out of the Dungeons");
  expect(card.props.image).toBe("51223bd0-ffd1-11df-a976-0801200c9000/Cards/51223bd0-ffd1-11df-a976-0801200c9131.jpg");
  card.flip();
  expect(card.props.image).toBe("51223bd0-ffd1-11df-a976-0801200c9000/Cards/51223bd0-ffd1-11df-a976-0801200c9131-B.jpg");
});

test('An encounter card has a common card back image', () => {
  let card = library.getCardByName("Endless Caverns");
  card.flip();
  expect(card.props.image).toBe("encounter_cardback.jpg");
});

test('A player card has a common card back image', () => {
  let card = library.getCardByName("Strider's Path");
  card.flip();
  expect(card.props.image).toBe("player_cardback.jpg");
});

test('User can exhaust a card', () => {
  let card = library.getCardByName("Dol Guldur Orcs");
  card.exhaust();
  expect(card.props.exhausted).toBeTruthy();
});

test('User can flip a card', () => {
  let card = library.getCardByName("Dol Guldur Orcs");
  expect(card.props.side).toBe("A");
  card.flip();
  expect(card.props.side).toBe("B");
  card.flip();
  expect(card.props.side).toBe("A");
});
